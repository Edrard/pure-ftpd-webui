...
");
		...
		elseif (isset ($_POST['install_mysql'])) {
			if (isset ($_POST['mysql_host']) && isset ($_POST['mysql_admin'])) {
				...
				function create_webui_user() {
					...
					$db_handler = mysqli_connect($mysql_host,$mysql_admin,$mysql_passwd);
					$link = $db_handler;
					mysqli_select_db($link, $mysql_database);
					$mysql_webui_create = mysqli_query($link, "GRANT ALL PRIVILEGES ON $mysql_database.* TO '$mysql_webui_user'@'$local_ip' IDENTIFIED BY '$mysql_webui_passwd';");
					if (!$mysql_webui_create) {
						echo("<p class=\"info\" align=\"left\">ERROR: Can not create user $mysql_webui_user.</p>\n");
					}
					$mysql_webui_create2 = mysqli_query($link, "GRANT ALL PRIVILEGES ON $mysql_database.* TO '$mysql_webui_user'@'$mysql_host' IDENTIFIED BY '$mysql_webui_passwd';");
				...
				function create_table_ftp() {
					...
					mysqli_select_db($link, $mysql_database);
					$users_table_create = mysqli_query($link, $ftp_users_table);
				...
				function create_table_settings() {
					...
					mysqli_select_db($link, $mysql_database);
					$settings_table_create = mysqli_query($link, $settings_table);
					if (!$settings_table_create) {
						...
						mysqli_select_db($link, $mysql_database);
						$settings_table_content_create = mysqli_query($link, $settings_table_content);
						if (!$settings_table_create) {
				...
				function create_table_webui() {
					...
					mysqli_select_db($mysql_database, $db_handler);
					$webui_table_create = mysqli_query($webui_users_table,$db_handler);
				...
				function create_database() {
					...
					$db_handler = mysqli_connect($mysql_host,$mysql_admin,$mysql_passwd);
					$db_create = FALSE;
					$db_create = mysqli_query("CREATE DATABASE $mysql_database",$db_handler);
					if (!$db_create) {
				...
				if (isset ($mysql_database) && $mysql_database != '') {
					$db_con = FALSE;
					$db_con = mysqli_select_db($mysql_database, $db_handler);
					if (!$db_con) {
						if (($err = mysqli_errno()) == 1049) {
							create_database();
							create_table_ftp();
						...
						$result = mysqli_query($test_ftp_table);
						if (!$result) {
							create_webui_user();
							create_table_ftp();
						}
						...
						$result = mysqli_query($test_settings_table);
						if (!$result) {
							create_table_settings();
						}
						else {
						...
						$result = mysqli_query($test_webui_table);
						if (!$result) {
							create_table_webui();
						}
						else {
		...
		elseif (isset ($_POST['add_webui_admin'])) {
			if (isset ($_POST['webui_admin']) && isset ($_POST['webui_admin_passwd']) && isset ($_POST['webui_admin_language'])) {
				...
				$result = mysqli_query($add_user_queue);
				if ($result != FALSE) {
